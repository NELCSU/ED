var App = (function (exports) {
  ...
  function initSankeyLegend(config) {
      var margin = { bottom: 10, left: 20, right: 20, top: 10 };
      ...
      function resize() {
          ...
          if (legend.classed("ready")) {
              ...
              var height_1 = rect.datum().height - 20;
              legend
                  .transition().duration(500)
              ...
              var height_2 = rect.datum().height - 20;
              legend
                  .transition().duration(500)
                  .attr("transform", function (d) {
                  d.y += height_2;
      ...
      function show() {
          ...
          var h = box.height;
          var w = box.width;
          var rh = config.legend.map(function (leg) { return leg.labels.length * 26; }).reduce(function (ac, le) { return ac + le; }, 0);
          var rw = 150;
          ...
          var voronoi = delaunay.voronoi([-1, -1, w - margin.left - margin.right + 1, h - margin.top - margin.bottom + 1]);
          ...
          close.append("title")
              .text("Close legend");
          var lasty = 10;
          config.legend.forEach(function (leg, n) {
              lasty += 30;
              // subtitle
              legend.append("text")
                  .attr("class", "contents")
              ...
              leg.colors.forEach(function (item, m) {
                  lasty += 17;
  ...
  function initPlayback(config) {
      var playShowHide = document.getElementById("PlaybackShowHide");
      playShowHide.addEventListener("input", function (e) {
  ...
  ***************************************************************************** */
  ...
  function initDataQualityChart(config) {
      ...
      window.addEventListener("data-quality", function () {
          ...
          var qt = "<div class=\"data-quality\">Data availability for <b>" + getScreenDate(config.querystring.day) + "</b>: ";
          if (state === 0) {
  ...
  function initPlayback$1(config) {
      var tip = document.querySelector("nel-tip");
      function run(nodes) {
          if (nodes.length === 0) {
              tip.show = false;
              return;
      ...
      window.addEventListener("node-playback", function (e) {
          run(e.detail);
      });
  }
