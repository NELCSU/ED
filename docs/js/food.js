...
d3.json(datapath + "json/stp.json", function (d) {
	...
	var setdays = function () {
		...
		JSZipUtils.getBinaryContent(filepath, function (err, rawdata) {
			...
			dayselect.node().value = Math.max(Math.min(prevday, Math.max.apply(null, Object.keys(interpolatedall))), Math.min.apply(null, Object.keys(interpolatedall)));
			...
			change = function () {
				...
				myhash = "&" + a.slice(a.length - 2, a.length) + "&" + dayselect.node().value + "&" + dropdown.node().value.stripSpaces();
